...
GirihCanvasHandler.prototype._getSVGSimilarChainClasses = function() {
    ...
    for (chainIndex = 0; chainIndex < this.girih.chains.length; chainIndex++) {
        ...
        //  set classification to zero padded length + 'L' if looping
        var classification = '00000' + chain.links.length;
        classification = classification.slice(-5);
        if (chain.isLoop) {
            classification = classification +'L';
        }
        //  add unique classification to list of classifications
    ...
    //generate CSS code for the chain classification
    css = "";
    for (classification of classifications) {
        // make the classification match that generated by the SVG routines
        classification = classification.replace(/^0+/,'');
        if (classification === '') {
            classification = '0';
        }
//**HERE DOCUMENT FOLLOWS***
        css += ".gfill.chain_length_"+ classification + ` {
